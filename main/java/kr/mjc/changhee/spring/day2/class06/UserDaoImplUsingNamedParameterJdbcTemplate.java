package main.java.kr.mjc.changhee.spring.day2.class06;
...
public class UserDaoImplUsingNamedParameterJdbcTemplate implements UserDao {
    ...
    private RowMapper<User> rowMapper = new BeanPropertyRowMapper<>(User.class); // 검색한 결과를 오브젝트에 맵핑하는 것 ResultSet -> Bean으로 변환
    ...
    public User getUser(int userId) {
        ...
        SqlParameterSource params = new MapSqlParameterSource("userId", userId); // 인자 1개 짜리
    ...
    public void addUser(User user) {
        ...
        SqlParameterSource params = new BeanPropertySqlParameterSource(user); // Bean(user)의 이름으로 맵을 만들어준다. sql을 수행하기위한 parameter자체를 map해서 넣는것 (Bean 객체로 파라미터 전달)
